...
//    // Open port
//    fd = open("/dev/cu.usbmodem146101", O_RDWR | O_NOCTTY | O_NDELAY);
//    if (fd == -1){
//        printf("Device cannot be opened.\n");
//        UE_LOG(LogTemp, Warning, TEXT("Port could not be opened %d"), fd);                       // If the device is not open, return -1
//    } else {
//        UE_LOG(LogTemp, Warning, TEXT("everything is fine"));
//    }
    UE_LOG(LogTemp, Warning, TEXT("HELLO WORLD"));
	Close();
    int response = RS232_OpenComportUE4(TCHAR_TO_ANSI(*port), baudrate);

	if (response)
	{
        UE_LOG(LogTemp, Warning, TEXT("UArduinoKitBP::Open -> Failed"));
        UE_LOG(LogTemp, Warning, TEXT("%d"), response);
